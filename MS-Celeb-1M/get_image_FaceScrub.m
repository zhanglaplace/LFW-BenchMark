function get_image_FaceScrub()

%%   由于内存原因，图像过大的会先进行resize处理
    root_folder = 'E:\datasets\FaceScrub';
    
    %image_list = get_image_list_in_folder(root_folder);
    %save FaceScrub_image_list.mat image_list;
    load FaceScrub_image_list.mat
    target_folder =  'E:\datasets\FaceScrub-112X96';
    ...
    %% 保存未检测到的人脸区域
    noface_file = 'E:\datasets\FaceScrub_noface.txt';
    noface_fid = fopen(noface_file,'w');
    
    image_id = 0;
    
    for image_id = 18048 : length(image_list)
        image_id = image_id +1;
        image_filename = image_list{image_id};
        try
        ...
        if isempty(boundingboxes)
            try
                fprintf(noface_fid,[image_filename 'no face detect']);
            catch
                disp(image_filename);
            end
            continue;
